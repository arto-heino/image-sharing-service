...
function ReadImage(id) {
    ...
    function myFunction(response) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            out += "<img class=\"img-responsive center-block\" src='images/" + arr[i].path;
            out += "' />";
        }
        out += "</div></div>";
        out += "<div class=\"row\">"
        out += "<div class=\"col-md-10\">Comment:"
        out += "<form class=\"form-horizontal\"><div class=\"form-group\"><label class=\"sr-only\" for=\"comment\">Email address</label><input type=\"text\" class=\"form-control\" id=\"comment\"></div><button type=\"submit\" class=\"btn btn-default\">comment</button></form>"
        out += "</div>"
        document.getElementById("images").innerHTML = out;
    }
}                    